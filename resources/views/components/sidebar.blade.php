<aside class="fixed inset-y-0 left-0 z-40 hidden w-64 border-r border-slate-200 bg-white lg:flex lg:flex-col">
    ...
    <nav class="flex-1 overflow-y-auto px-4 py-6">
        ...
        @if (session('keluarga_level_akses') === 'pemilik' || session('keluarga_level_akses') === 'editor')
            ...
            <div class="mb-6">
                ...
                <a href="{{ route('undangan.index') }}"
                    class="mb-1 flex items-center justify-between rounded-xl px-3 py-3 text-sm transition
                        {{ request()->routeIs('undangan.*')
                            ? 'bg-blue-50 font-semibold text-blue-600'
                            : 'text-slate-600 hover:bg-slate-50' }}">
                    ...
                    {{-- Badge --}}
                    @if (isset($undanganMenunggu) && $undanganMenunggu > 0)
                        <span
                            ...
                            {{ $undanganMenunggu > 99 ? '99+' : $undanganMenunggu }}
                        </span>
                    @endif
